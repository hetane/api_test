...
public abstract class AbstractContentProvider implements ITreeContentProvider
{
   ...
   protected final ListenerSupport<ModelItem,PropertyChangeListener> nameListeners;
   ...
   {
      projectListeners = new ListenerSupport<Project,ProjectListener>(
            new InternalProjectListener())
            {
      ...
      interfaceListeners = new ListenerSupport<Interface,InterfaceListener>(
      ...
      testSuiteListeners = new ListenerSupport<TestSuite,TestSuiteListener>(
            new InternalTestSuiteListener())
            {
      ...
      mockServiceListeners = new ListenerSupport<MockService,MockServiceListener>(
            new InternalMockServiceListener())
            {
      ...
      nameListeners = new ListenerSupport<ModelItem,PropertyChangeListener>(
   ...
   public Object[] getChildren(Object parent)
   {
      ...
      if(parent instanceof Project)
      {
         ...
         for(int i = 0; i < interfaceCount; i++)
         {
         	...
         	elements[i] = iface;
            nameListeners.addListener(elements[i]);
         }
         for(int i = 0; i < testSuiteCount; i++)
         {
            ...
            nameListeners.addListener(elements[index]);
         }
      ...
      else if(parent instanceof TestCase)
      {
      	...
      	for(ModelItem child : elements) {
      		nameListeners.addListener(child);
      	}*/
   ...
   public boolean hasChildren(Object element)
   {
      ...
      {
//         return ((TestCase)element).getTestStepCount() + ((TestCase)element).getLoadTestCount() > 0;
         return true;
      }
      else if(element instanceof TestStepsElement)
      {
...
//         Object element = iface.getProject());
      ...
      {
// TODO Eclipse jbossws: Adapt IFile "soapui-project.xml" to Project to handle this. 
// When the content is actually a soapui-project.xml file,
// the view will not be updated if we send the Project.
//         Object element = iface.getProject());
         Object element = null;

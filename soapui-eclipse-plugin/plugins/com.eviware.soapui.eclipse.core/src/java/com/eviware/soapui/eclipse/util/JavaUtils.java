...
public class JavaUtils
{
   ...
   /**
    * Get the classpath to the selected class (the compiled class) relative to the project.
    * TODO Eclipse wstools: The user must build the project before this command.
    * 
   ...
   public static String getPackageName(String fullName, String className)
   {
      if(fullName.length() > ("." + className).length())
      {
         return fullName.substring( 0,
            fullName.length() - ("." + className).length() );
      }
      else
      {
         return "";

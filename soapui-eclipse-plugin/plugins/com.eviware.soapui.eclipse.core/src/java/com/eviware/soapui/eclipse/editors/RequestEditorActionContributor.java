...
//            setEnabled(true);
         ...
         final ModelItemEditor messageEditor =
            (editorPart instanceof ModelItemEditor ? 
                  (ModelItemEditor) editorPart : null);
...
//                  messageEditor.setStatusText(message, status != Status.ERROR);
   ...
   private class SubmitAction extends Action
   {
      SubmitAction()
      {
         super("Submit request to specified endpoint URL",
               Images.getImageDescriptor("submit_request.gif"));
      }
   ...
   private class RecreateAction extends Action
   {
      RecreateAction()
      {
         super("Recreates a default request from the schema",
               Images.getImageDescriptor("recreate_request.gif"));
      }
      
      @Override
      public void run()
      {
         boolean createOptional = UISupport.getDialogs().confirm(
               "Create optional elements in schema?", "Recreate Request");
         
         // TODO Eclipse: This may fail (connection refused) in another thread (SwingWorker)
         ...
         {
            boolean option = UISupport.getDialogs().confirm(
                  "Keep existing values?", "Recreate Request");
            if(option)
            {
   ...
   private class SetEndpointAction extends Action
   {
      SetEndpointAction()
      {
         super("Select endpoint", 
               Images.getImageDescriptor("set_endpoint.gif"));
      }
      ...
      public void run()
      {
         ...
         String newEndpoint = UISupport.getDialogs().prompt(
               "Enter the new endpoint:", "Select endpoint", oldEndpoint);
   ...
   private class CancelAction extends Action
   {
      CancelAction()
      {
         super("Aborts ongoing request",
               Images.getImageDescriptor("cancel_request.gif"));
         setEnabled(false);
      }
   ...
   private final IAction[] actions = new IAction[] {
         submitAction, cancelAction, recreateAction, setEndpointAction
   };

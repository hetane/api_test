...
public class AddWadlToSoapUIProjectAction implements IObjectActionDelegate
{
	 ...
	 */
	public AddWadlToSoapUIProjectAction()
	{
		super();
	}
		...
		try
		{
			pmd.run( false, false, new IRunnableWithProgress()
			{
				public void run( IProgressMonitor monitor )
				{
					...
					try
					{
						RestService restService = ( RestService )project.addNewInterface( project.getName(),
								RestServiceFactory.REST_TYPE );
						new WadlImporter( restService ).initFromWadl( file.getLocationURI().toString() );
						// WsdlInterface[] iface = WsdlImporter.importWsdl(project,

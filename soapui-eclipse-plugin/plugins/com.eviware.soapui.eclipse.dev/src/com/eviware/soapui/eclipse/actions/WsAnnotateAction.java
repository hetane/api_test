...
public class WsAnnotateAction implements IObjectActionDelegate
{
	...
	public void run( IAction action )
	{
		...
		try
		{
			...
			String endpointInterface = codeGen
					.getAnnotationParamString( type, "javax.jws.WebService", "endpointInterface" );
			...
			String parameterStyle = codeGen
					.getAnnotationParamString( type, "javax.jws.soap.SOAPBinding", "parameterStyle" );
			...
			values.put( WS_NAMESPACE, targetNamespace == null ? "urn:"
					+ selectedClass.getPackageFragment().getElementName() : targetNamespace );

			values.put( WS_PORTNAME, portName );
			values.put( WS_WSDLLOCATION, wsdlLocation );
			values.put( WS_ENDPOINTINTERFACE, endpointInterface );

			values.put( WS_BINDINGSTYLE, serviceStyle == null
					|| serviceStyle.equals( "javax.jws.soap.SOAPBinding.Style.DOCUMENT" ) ? "Document" : "RPC" );
			values.put( WS_BINDINGUSE,
					useStyle == null || useStyle.equals( "javax.jws.soap.SOAPBinding.Use.LITERAL" ) ? "LITERAL" : "ENCODED" );
			values.put( WS_BINDINGPARAMETERSTYLE, parameterStyle == null
					|| parameterStyle.equals( "javax.jws.soap.SOAPBinding.ParameterStyle.WRAPPED" ) ? "WRAPPED" : "BARE" );
			...
			if( values.get( WS_BINDINGSTYLE ).equalsIgnoreCase( "Document" ) )
				...
				codeGen.addAnnotationParam( annotation, "style", codeGen.getAst().newName(
						"javax.jws.soap.SOAPBinding.Style.RPC" ), null );

			if( values.get( WS_BINDINGUSE ).equals( "LITERAL" ) )
				codeGen.removeAnnotationParam( annotation, "use" );
			else
				codeGen.addAnnotationParam( annotation, "use", codeGen.getAst().newName(
						"javax.jws.soap.SOAPBinding.Use.ENCODED" ), null );

			if( values.get( WS_BINDINGPARAMETERSTYLE ).equals( "WRAPPED" ) )
				codeGen.removeAnnotationParam( annotation, "parameterStyle" );
			else
				codeGen.addAnnotationParam( annotation, "parameterStyle", codeGen.getAst().newName(
						"javax.jws.soap.SOAPBinding.ParameterStyle.BARE" ), null );
	...
	protected XFormDialog buildDialog()
	{
		...
		mainForm.addTextField( WS_ENDPOINTINTERFACE, "The endpoint interface implemented by this service",
				XForm.FieldType.TEXT );

		mainForm.addSeparator();

		mainForm.addComboBox( WS_BINDINGSTYLE, new String[] { "Document", "RPC" }, "The encoding style to use" );
		mainForm.addComboBox( WS_BINDINGUSE, new String[] { "LITERAL", "ENCODED" }, "The formatting style to use" );
		mainForm.addComboBox( WS_BINDINGPARAMETERSTYLE, new String[] { "WRAPPED", "BARE" }, "The parameter style to use" );

		mainForm.addSeparator();
		mainForm.addTextField( WS_HANDLERFILE, "The handlerfile", XForm.FieldType.PROJECT_FILE );

		return builder.buildDialog( builder.buildOkCancelActions(), "Specify values for JAX-WS WebService annotations",
				UISupport.TOOL_ICON );
	}
